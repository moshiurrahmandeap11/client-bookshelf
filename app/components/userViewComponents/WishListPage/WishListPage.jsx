...
const WishListPage = () => {
  ...
  // লোকাল স্টোরেজ থেকে লাইক করা বই আইডি পাওয়া
  ...
  // উইশলিস্ট আইটেম ফেচ করা
  const fetchWishlistItems = async () => {
    try {
      setLoading(true);
      const likedBookIds = getLikedBookIds();
      const likedIds = Object.keys(likedBookIds).filter(id => likedBookIds[id] === true);
      ...
      // প্রতিটি বইয়ের ডিটেইলস ফেচ করা
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-8 text-center sm:text-left">
          <div className="inline-flex items-center gap-2 bg-amber-500/10 backdrop-blur-sm border border-amber-500/20 rounded-full px-4 py-1.5 mb-4">
            ...
            <span className="text-xs sm:text-sm text-amber-400 font-medium">My Collection</span>
          </div>
          <h1 className="text-3xl sm:text-4xl lg:text-5xl font-bold">
            <span className="text-white">My </span>
            <span className="bg-gradient-to-r from-amber-400 via-orange-400 to-amber-500 bg-clip-text text-transparent">Wishlist</span>
          </h1>
        ...
        {wishlistItems.length === 0 ? (
          <div className="text-center py-20">
            ...
            <p className="text-gray-400 mb-6">Start adding books you love to your wishlist</p>
            <Link
              href="/browse"
          ...
          <>
                    ...
                    <tr key={book._id} className="border-b border-white/5 hover:bg-white/5 transition-colors group">
                      ...
                      <td className="py-4">
                        <div className="flex items-center gap-3">
                          {book.thumbnail ? (
                            <Image src={book.thumbnail} alt={book.title} width={40} height={50} className="w-10 h-14 rounded object-cover" />
                          ) : (
                          ...
                          <div>
                            ...
                            <p className="text-gray-500 text-xs lg:hidden">{book.authorName}</p>
                          </div>
                        </div>
                      </td>
                      <td className="py-4">
                        <span className="text-gray-300 text-sm hidden lg:inline">{book.authorName}</span>
                      </td>
                      <td className="py-4">
                        <div>
                          <p className="text-amber-400 font-semibold">${book.discountPrice}</p>
                          {book.price > book.discountPrice && <p className="text-gray-500 text-xs line-through">${book.price}</p>}
                        </div>
                      </td>
                      <td className="py-4">
                      ...
                      <td className="py-4">
                        <div className="flex items-center gap-2">
                          <Link href={`/books/${book._id}`} className="p-1.5 rounded-lg bg-blue-500/20 text-blue-400 hover:bg-blue-500/30 transition-colors" title="View Details">
                            <FiEye className="w-4 h-4" />
                          </Link>
                          <button onClick={() => handleAddToCart(book)} className="p-1.5 rounded-lg bg-emerald-500/20 text-emerald-400 hover:bg-emerald-500/30 transition-colors" title="Add to Cart">
                            <FiShoppingCart className="w-4 h-4" />
                          </button>
                          <button onClick={() => removeFromWishlist(book._id, book.title)} className="p-1.5 rounded-lg bg-red-500/20 text-red-400 hover:bg-red-500/30 transition-colors" title="Remove from Wishlist">
            ...
            {/* কার্ড ভিউ (মোবাইল ও ট্যাবলেটের জন্য) */}
            <div className="lg:hidden space-y-4">
              {wishlistItems.map((book) => (
                <div key={book._id} className="bg-gradient-to-br from-white/5 to-white/0 backdrop-blur-sm rounded-2xl border border-white/10 p-4">
                  <div className="flex gap-4">
                    ...
                    <div className="flex-1">
                      <h3 className="text-base font-bold text-white mb-0.5 line-clamp-1">{book.title}</h3>
                      ...
                      <div className="mt-1">
                        <p className="text-amber-400 font-bold text-base">${book.discountPrice}</p>
                        {book.price > book.discountPrice && <p className="text-gray-500 text-xs line-through">${book.price}</p>}
                      </div>
                      <div className="flex gap-2 mt-2">
                        <Link href={`/books/${book._id}`} className="flex-1 px-2 py-1.5 bg-blue-500/20 text-blue-400 rounded-lg text-center text-xs hover:bg-blue-500/30 transition">
                          View
                        </Link>
                        <button onClick={() => handleAddToCart(book)} className="flex-1 px-2 py-1.5 bg-emerald-500/20 text-emerald-400 rounded-lg text-xs hover:bg-emerald-500/30 transition">
                          Cart
                        </button>
                        <button onClick={() => removeFromWishlist(book._id, book.title)} className="px-2 py-1.5 bg-red-500/20 text-red-400 rounded-lg hover:bg-red-500/30 transition">
          ...
          <div className="mt-8 p-4 bg-white/5 rounded-xl border border-white/10">
            <div className="flex flex-wrap items-center justify-between gap-4">
              <div className="flex items-center gap-2">
                ...
                <span className="text-white">{wishlistItems.length} books in wishlist</span>
              </div>
              <div className="flex items-center gap-2">
                <FiShoppingCart className="w-5 h-5 text-emerald-400" />
                <span className="text-gray-400">Total value: ${wishlistItems.reduce((sum, b) => sum + (b.discountPrice || 0), 0).toFixed(2)}</span>
              </div>
              <Link href="/browse" className="text-amber-400 hover:text-amber-300 text-sm">
                Continue Shopping →
              </Link>
            </div>
          </div>
