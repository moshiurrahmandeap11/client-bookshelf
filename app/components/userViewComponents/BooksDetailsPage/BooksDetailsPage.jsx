...
const BooksDetailsPage = () => {
      ...
      if (response.data.success) {
        ...
        if (response.data.data.category?.slug) {
          const relatedRes = await axiosInstance.get(
            `/books?category=${response.data.data.category.slug}&limit=4`,
          );
          if (relatedRes.data.success) {
            setRelatedBooks(
              relatedRes.data.data.filter((b) => b._id !== bookId),
            );
          }
        }
      }
  ...
  const renderStars = (rating, size = "w-4 h-4") => {
    ...
    for (let i = stars.length; i < 5; i++) {
      stars.push(
        <FaStar key={`empty-${i}`} className={`${size} text-gray-600`} />,
      );
    }
    return stars;
  };
    ...
    return (
      <div className="min-h-screen bg-gradient-to-b from-[#1C1712] via-[#2A2219] to-[#1C1712] flex items-center justify-center">
        <div className="text-center">
          ...
          <Link href="/browse" className="text-amber-400 hover:text-amber-300">
            Browse Books
          </Link>
        </div>
      </div>
        ...
        <div className="grid lg:grid-cols-2 gap-8 lg:gap-12 mb-12">
            ...
            <div className="relative h-96 lg:h-[500px] rounded-2xl overflow-hidden bg-white/5 border border-white/10">
              {allImages[activeImage] ? (
                ...
                <div className="w-full h-full flex items-center justify-center">
                  <FiBookOpen className="w-20 h-20 text-gray-600" />
                </div>
              )}
            </div>
            {allImages.length > 1 && (
              <div className="flex gap-2 mt-4 overflow-x-auto pb-2">
                {allImages.map((img, idx) => (
                  <button
                    ...
                    className={`relative w-20 h-20 rounded-lg overflow-hidden border-2 transition-all ${activeImage === idx ? "border-amber-500" : "border-white/20"}`}
                  >
                    <Image
                      src={img}
                      alt={`View ${idx + 1}`}
          ...
          <div>
            <div className="flex items-start justify-between gap-4">
              <div>
                <span className="inline-block px-3 py-1 bg-amber-500/20 text-amber-400 rounded-full text-xs mb-3">
                  {book.category?.name}
                </span>
              ...
              <button
                onClick={handleLike}
                className="p-3 rounded-full bg-white/10 hover:bg-red-500/20 transition-colors"
              >
                {liked ? (
            ...
            <div className="grid grid-cols-2 gap-4 mt-6 p-4 bg-white/5 rounded-xl">
              <div>
                ...
                <p className="text-white">{book.publisher || "N/A"}</p>
              </div>
              <div>
              ...
              <div>
                ...
                <p className="text-white">{book.isbn || "N/A"}</p>
              </div>
              <div>
              ...
              <div>
                ...
                <p
                  className={book.stock > 0 ? "text-green-400" : "text-red-400"}
                >
            ...
            {book.stock > 0 && (
              <div className="mt-6">
                ...
                <button
                  ...
                  className="w-full py-3 bg-gradient-to-r from-amber-500 to-orange-600 text-white font-semibold rounded-xl hover:from-amber-600 hover:to-orange-700 transition-all duration-300 flex items-center justify-center gap-2 disabled:opacity-50"
                >
        ...
        <div className="border-t border-white/10 pt-8">
          ...
          <div className="bg-white/5 rounded-2xl p-6 mb-8">
            ...
            <button
              ...
              className="mt-4 px-6 py-2 bg-amber-500 text-white rounded-xl hover:bg-amber-600 transition-colors disabled:opacity-50 flex items-center gap-2"
            >
              {submittingReview ? (
