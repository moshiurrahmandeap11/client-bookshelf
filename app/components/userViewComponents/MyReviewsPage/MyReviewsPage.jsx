  ...
  const handleUpdateReview = async () => {
    ...
    try {
      const response = await axiosInstance.put(
        `/books/${editingReview.bookId}/reviews/${editingReview.id}`,
        {
          rating: editRating,
          comment: editComment,
        },
      ...
      if (result.isConfirmed) {
        try {
          const response = await axiosInstance.delete(
            `/books/${review.bookId}/reviews/${review.id}`,
          );
          if (response.data.success) {
            Swal.fire({
              icon: "success",
    ...
    <div className="min-h-screen bg-gradient-to-b from-[#1C1712] via-[#2A2219] to-[#1C1712] py-8 sm:py-12">
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-8">
          ...
          <p className="text-gray-400 mt-2">Reviews you&apos;ve written for books</p>
        </div>
        ...
        {reviews.length === 0 ? (
          ...
          <div className="space-y-4">
            {reviews.map((review) => (
              <div
                key={review.id}
                className="bg-gradient-to-br from-white/5 to-white/0 backdrop-blur-sm rounded-2xl border border-white/10 p-5 hover:border-amber-500/30 transition-all duration-300"
              >
            ...
            <div className="bg-gradient-to-br from-[#2A2219] to-[#1C1712] rounded-2xl w-full max-w-md border border-white/20 shadow-2xl">
              ...
              <div className="flex gap-3 p-5 pt-0">
                ...
                <button
                  ...
                  className="flex-1 px-4 py-2 bg-gradient-to-r from-amber-500 to-orange-600 text-white rounded-xl hover:from-amber-600 hover:to-orange-700 transition-all duration-300"
                >
                  Update Review
                </button>
              </div>
